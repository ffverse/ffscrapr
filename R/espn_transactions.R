...
ff_transactions.espn_conn <- function(conn, limit = 1000, ...){
  ...
  xff <- list(topics = list(filterType = list(value = list("ACTIVITY_TRANSACTIONS")),
                            limit = limit,
  ...
  all_transactions <- transactions_response %>%
    ...
    dplyr::mutate(timestamp = lubridate::as_datetime(.data$date/1000),
                  x = NULL) %>%
    tidyr::unnest_longer("messages") %>%
    tidyr::hoist("messages","from","for","to","messageTypeId","player_id"="targetId") %>%
    dplyr::mutate(t = .espn_activity_map()[as.character(.data$messageTypeId)],
                  franchise_id = dplyr::case_when(.data$messageTypeId == 244 ~ .data$from,
                                                  .data$messageTypeId == 239 ~ .data[['for']],
                                                  TRUE ~ .data$to),
                  bbid_spent = dplyr::case_when(.data$messageTypeId == 180 ~ .data$from),
                  trade_partner = dplyr::case_when(.data$messageTypeId == 244 ~ .data$to,
                                                   TRUE ~ NA_integer_),
                  messages = NULL,
                  date = NULL,
                  `for` = NULL,
  ...
  if("TRADE" %in% all_transactions$type) {
    ...
    trade_transactions <- all_transactions %>%
      ...
      dplyr::select(trade_partner = .data$franchise_id,
