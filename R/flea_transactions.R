...
.flea_team_transactions <- function(franchise_id,conn){
  ...
  while(!is.null(result_offset)){
    ...
    results <- fleaflicker_getendpoint(endpoint = "FetchLeagueTransactions",
                                       sport = "NFL",
                                       league_id = conn$league_id,
                                       team_id = franchise_id,
...
.flea_transactions_add <- function(raw_transactions){
  ...
  if(nrow(adds)==0) return(NULL)

  adds <- adds %>%
    tidyr::hoist('franchise',"franchise_id"="id","franchise_name"="name") %>%
    dplyr::mutate(transaction = purrr::map(.data$transaction,purrr::pluck,"player","proPlayer"),
                  type = "free_agency",
                  type_desc = "added") %>%
...
.flea_transactions_trade <- function(conn){
  ...
  while(!is.null(result_offset)){
    ...
    results <- fleaflicker_getendpoint(endpoint = "FetchTrades",
                                       sport = "NFL",
                                       filter = "TRADES_COMPLETED",
                                       league_id = conn$league_id,
                                       result_offset = 80) %>%
  ...
  pick_trades <- trades %>%
    ...
    dplyr::mutate(player_id = glue::glue("{.data$season}_{.data$round}_{.data$original_id}"),
  ...
  crossing <- df_trades %>%
    ...
    dplyr::mutate(type_desc = dplyr::case_when(.data$type == "free_agency" ~ "dropped",
                                               TRUE ~ .data$type_desc)) %>%
    dplyr::select(dplyr::any_of(c(
      "timestamp",
      "type",
