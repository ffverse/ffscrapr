...
#' @examples
#' \donttest{
#' test_conn <- sleeper_connect(league_id = "653543448376320000", season = 2021)
#' test_starters <- ff_starter_positions(jml_conn)
#' }
#'
#' @export
ff_starter_positions.sleeper_conn <- function(conn, ...) {
  ...
  df_positions %>%
    ...
    offense_starters = sum(
      .data$pos %in% c("QB","RB","WR","TE", "FLEX","WRRB_FLEX", "REC_FLEX","SUPER_FLEX") * .data$min, na.rm = TRUE),
