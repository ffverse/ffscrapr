with_mock_api({
  test_that("ff_scoringhistory returns a tibble of player scores", {
    ...
    if (!identical(Sys.getenv("MOCK_BYPASS"), "true")) {
      testthat::local_mock(
        nflfastr_weekly = function() readRDS("ffscrapr-tests-1.4.5/gh_nflfastr/player_stats.rds"),
        nflfastr_rosters = function(seasons) {
          purrr::map_df(seasons, ~ readRDS(glue::glue("ffscrapr-tests-1.4.5/gh_nflfastr/roster_{.x}.rds")))
        }
      )
    }
