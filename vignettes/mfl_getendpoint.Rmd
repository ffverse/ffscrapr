---
title: "mfl_getendpoint"
author: "Tan Ho"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{mfl_getendpoint}
...
The [MFL API is extensive](https://api.myfantasyleague.com/2020/api_info?STATE=details), and I haven't written out a function for every single one of the endpoints. If there is something you'd like to access, you can use the lower-level "`mfl_getendpoint`" function to create a GET request and access the data, while still using the authentication and rate-limiting features I've already created. 
...
fog_tradebait <- mfl_getendpoint(fog, "tradeBait", INCLUDE_DRAFT_PICKS = 1) %>% 
  ...
  dplyr::left_join(
    ff_franchises(ssb_2019) %>% dplyr::select("franchise_id","franchise_name"),
    by = c("franchise_id")
  ) %>% 
  dplyr::left_join(
